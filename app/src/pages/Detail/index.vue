        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  @click="
                    changeActice(saleAttrValue, saleAttr.spuSaleAttrValueList)
                  "
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 之前路由跳轉,A路由跳轉至B路由,但在加入購物車,進行路由跳轉前,要先發請求-->
                <!-- 把購買的產品信息,通過請求服務器,並讓服務器進行相應的存儲 -->
...
export default {
  ...
  computed: {
    ...
    //給子組件的數據
    skuImageList() {
      //如果服務器數據沒有回來,至少skuInfo是個空對象
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    //產品的售賣屬性值切換高亮
    changeActice(saleAttrValue, arr) {
      //遍歷全部售賣屬性值 isChecked 為0,沒有高亮
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      //點擊的該售賣屬性值,有高亮
    ...
    async addShopCar() {
      try {
        //1.發請求---將產品信息加入到數據庫(通知服務器)
        await this.$store.dispatch("AddOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //2.服務器存儲成功---進行路由跳轉傳遞參數
        //3.在路由跳轉的時候,需要將產品的信息帶給下一級的路由組件
        /* 一些簡單的數據 例:skuNum,可通過query參數傳遞
           而產品信息的數據比較複雜時【例:skuInfo】適合通過會話存儲 (瀏覽器關閉就會消失)
           注意:本地存儲|會話存儲,一般儲存的是字串
        */
